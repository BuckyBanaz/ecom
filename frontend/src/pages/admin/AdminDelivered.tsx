    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-1">
        <h2 className="text-2xl font-semibold flex items-center gap-2">
          <CheckCircle className="h-6 w-6 text-primary" /> {t("admin_orders_delivered.title")}
        </h2>
        <p className="text-sm text-muted-foreground">
          {t("admin_orders_delivered.subtitle")}
        </p>
      </div>
      ...
      <div className="relative max-w-md">
        <Input
          ...
          placeholder={t("admin_orders_delivered.search_placeholder")}
        ...
        <table className="w-full text-sm text-left border-collapse">
          <thead>
            <tr className="border-b bg-muted/40 text-muted-foreground font-medium text-xs">
              <th className="p-4">{t("admin_orders_delivered.table_order_id")}</th>
              <th className="p-4">{t("admin_orders_delivered.table_customer")}</th>
              <th className="p-4">{t("admin_orders_delivered.table_carrier")}</th>
              <th className="p-4">{t("admin_orders_delivered.table_tracking")}</th>
              <th className="p-4">{t("admin_orders_delivered.table_status")}</th>
              <th className="p-4 text-right">{t("admin_orders_delivered.table_actions")}</th>
            </tr>
          </thead>
            ...
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={6} className="p-8 text-center text-muted-foreground">
                  {t("admin_orders_delivered.empty")}
                </td>
              </tr>
            ) : (
              filtered.map((o) => {
                return (
                  <tr key={o.id} className="hover:bg-muted/20 transition-colors">
                    ...
                    <td className="p-4 text-right">
                      ...
                      >
                        <Truck className="h-3.5 w-3.5" /> {t("admin_orders_delivered.button_details")}
                      </Button>
                    </td>
                  </tr>
                );
