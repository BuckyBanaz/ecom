# v1.4 — CI/CD Implementation Plan

> **Status:** **Deprecated (June 2026)** — Jenkins removed from production stack. Deploy via SSH: `BRANCH=code-deploy bash scripts/deploy.sh`. See `docs/vps_operations_runbook.md`.

> **Branch:** `v1.4`  
> **Target server:** Hostinger VPS `187.124.21.137` → `/opt/ecom`  
> **Live domains:** `schipenster.com` / `api.schipenster.com`  
> **Status:** Implemented — CI workflow + Jenkins pipeline files in repo; run `scripts/setup-jenkins.sh` on VPS once
...
├── jenkins/Dockerfile             ✅
└── docs/v1.4-cicd-jenkins-plan.md
```
...
## 11. VPS setup (run once after push)

```bash
ssh root@187.124.21.137
cd /opt/ecom
git pull origin v1.4
chmod +x scripts/*.sh
...
Then configure Jenkins job (see `setup-jenkins.sh` output).

**Jenkins URL (no public IP:8080):**
- DNS: `jenkins.schipenster.com` → VPS IP (A record)
- Access: `https://jenkins.schipenster.com`
- In Jenkins: Manage Jenkins → System → Jenkins URL = `https://jenkins.schipenster.com/`

**Manual deploy (without Jenkins):**
```bash
cd /opt/ecom && BRANCH=v1.4 bash scripts/deploy.sh
```

**Rollback:**
```bash
cd /opt/ecom && bash scripts/rollback.sh <commit-sha>
```

*Implemented on branch `v1.4`.*
